...
// Display news content using Promises
...
document.querySelector('button').addEventListener('click', () => {
  ...
  // Fetch news data using Promises
  getNewsData(newsApiUrl)
    .then(displayNews)
    .catch(error => {
      console.error(error);
      newsContainer.innerHTML = '<p>Failed to fetch news data.</p>';
    })
    .finally(() => {
      document.querySelector('button').textContent = 'View all the news';
    });
});
